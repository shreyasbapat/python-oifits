...
class OI_VIS:
    ...
    def __repr__(self):
        ...
        if self.station[0] and self.station[1]:
            ...
            baselinename = ''
        return '%s %s%s: %d point%s (%d masked), B = %5.1f m, PA = %5.1f deg, <V> = %4.2g' % (
        self.target.target, self.timeobs.strftime('%F %T'), baselinename, len(self.visamp), _plurals(len(self.visamp)),
        np.sum(self.flag), np.sqrt(self.ucoord ** 2 + self.vcoord ** 2),
        np.arctan(self.ucoord / self.vcoord) * 180.0 / np.pi % 180.0, meanvis)
...
class OI_VIS2:
    ...
    def __repr__(self):
        ...
        self.target.target, self.timeobs.strftime('%F %T'), baselinename, len(self.vis2data),
        _plurals(len(self.vis2data)), np.sum(self.flag), np.sqrt(self.ucoord ** 2 + self.vcoord ** 2),
        np.arctan(self.ucoord / self.vcoord) * 180.0 / np.pi % 180.0, meanvis)
...
class OI_T3:
    ...
    def __repr__(self):
        ...
        self.target.target, self.timeobs.strftime('%F %T'), baselinename, len(self.t3amp), _plurals(len(self.t3amp)),
        np.sum(self.flag), np.sqrt(self.u1coord ** 2 + self.v1coord ** 2),
        np.sqrt(self.u2coord ** 2 + self.v2coord ** 2), meant3)
...
class OI_ARRAY:
    ...
    def __init__(self, frame, arrxyz, stations=()):
        ...
        for station in stations:
            ...
            self.station = np.append(self.station,
                                     OI_STATION(tel_name=tel_name, sta_name=sta_name, diameter=diameter, staxyz=staxyz))
    ...
    def __repr__(self):
        ...
        self.latitude.asdms(), self.longitude.asdms(), self.altitude, len(self.station), _plurals(len(self.station)))
