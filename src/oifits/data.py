...
class Data:
    ...
    def isvalid(self):
        ...
        for vis in self.vis:
            ...
            if (len(vis.visamp) != nwave) or (len(vis.visamperr) != nwave) or (len(vis.visphi) != nwave) or (
                    len(vis.visphierr) != nwave) or (len(vis.flag) != nwave):
                errors.append(
        ...
        for t3 in self.t3:
            ...
            if (len(t3.t3amp) != nwave) or (len(t3.t3amperr) != nwave) or (len(t3.t3phi) != nwave) or (
                    len(t3.t3phierr) != nwave) or (len(t3.flag) != nwave):
                errors.append(
                    "Data size mismatch for visibility measurement 0x%x (wavelength table has a length of %d)" % (
                    id(vis), nwave))
    ...
    def info(self, recursive=True, verbose=0):
        ...
        if self.wavelength:
            ...
            len(self.wavelength), _plurals(len(self.wavelength)), wavelengths, _plurals(wavelengths))
        if self.target.size:
            if recursive:
                print
                "===================================================================="
        ...
        if self.array:
            ...
            print
            "%d array%s with %d station%s" % (len(self.array), _plurals(len(self.array)), stations, _plurals(stations))
        if self.vis.size:
            if recursive:
                print
                "===================================================================="
            ...
            for t3 in self.t3:
                ...
                if t3.station[0] and t3.station[1] and t3.station[2]:
                    data['sta_index'].append(
                        [stationmap[id(t3.station[0])], stationmap[id(t3.station[1])], stationmap[id(t3.station[2])]])
                else:
